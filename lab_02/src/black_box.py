...
class DesmosStepFunction(BaseFunction):
    """
    Ступенчатая функция из Desmos.
    """
    def __init__(self):
        super().__init__()
        self.dim = 2
    ...
    def __call__(self, x: Vector) -> float:
        self.f_calls += 1
        if len(x) != self.dim:
            raise ValueError(f"Вектор должен содержать {self.dim} координат.")
        
        # Безопасно извлекаем скаляры через нашу функцию
        x_val = to_real_value(x[0])
        y_val = to_real_value(x[1])
        
        # Расчет по формуле с внедренными разрывами
        term1 = x_val * (round(math.sin(10 * y_val)) + 2)
        part1 = (term1**2 + y_val - 10)**2
        
        term2 = y_val * (round(math.sin(7 * x_val)) + 2)
        part2 = (x_val + term2**2 - 7)**2
        
        return part1 + part2

    def gradient(self, x: Vector) -> Vector:
        self.grad_calls += 1
        # На разрывах градиента нет, на террасах он равен нулю.
        return [0.0, 0.0]
...
class RastriginFunction(BaseFunction):
    """
    Функция Растригина. Тест на застревание в локальных минимумах.
    """
    ...
    def __call__(self, x: Vector) -> float:
        ...
        for i in range(self.dim):
            xi = to_real_value(x[i])
            result += xi**2 - self.A * math.cos(2 * math.pi * xi)
    ...
    def gradient(self, x: Vector) -> Vector:
        ...
        for i in range(self.dim):
            ...
            gi = 2 * xi + 2 * math.pi * self.A * math.sin(2 * math.pi * xi)
            grad.append(gi)
        return grad

    def hessian(self, x: Vector) -> list[Vector]:
        self.hessian_calls += 1
        return [[0.0] * self.dim for _ in range(self.dim)]